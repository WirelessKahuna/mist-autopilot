...
class SecureScopeModule(BaseModule):
    ...
    async def analyze(self, org_id: str, sites: list[dict], client: MistClient) -> ModuleOutput:
        ...
        for psk, ssid_set in reused_psks.items():
            ...
            # Suppress only if ALL names in the group are band variants of the
            # same base (e.g. "Mist APoS 2.4", "Mist APoS 5", "Mist APoS 6").
            # A band variant ends with a bare band number (2.4, 5, 6) after the
            # final space. Mixed groups (APoS + AP12 + MistAI Guest) must NOT
            # be suppressed — they contain unrelated SSIDs.
            ...
            bases = {_base(s) for s in ssid_list}
            if len(bases) == 1:
                continue  # pure band variants of one SSID — expected

            reuse_severity = Severity.critical if len(ssid_list) > 2 else Severity.warning
            f = Finding(
                severity=reuse_severity,
                title=f"PSK reuse — same passphrase on {len(ssid_list)} differently-named SSIDs",
                detail=(
