...
class ConfigDriftModule(BaseModule):
    ...
    async def analyze(self, org_id: str, sites: list[dict], client: MistClient) -> ModuleOutput:
        from .wlan_utils import build_site_wlan_map, build_cross_site_index

        # 1. Fetch all WLAN data in parallel
        # NOTE: We do NOT fetch org-level WLANs separately. The site WLAN endpoint
        # returns the fully resolved set for each site — template WLANs with
        # applies_to scope and exclusions already evaluated by Mist's backend.
        ...
        # WLAN template SSIDs are already in the site lists — use for drift skip check
        ...
        # Build annotated per-site map
        annotated_site_wlans = build_site_wlan_map(sites, site_wlan_lists)
        for sid, wlan_list in annotated_site_wlans.items():
            for wlan in wlan_list:
                wlan["_source"] = "site"

        # Build cross-site index — deduplicates template WLANs by WLAN ID
        # so the same SSID from a template doesn't create false drift findings
        cross_site = build_cross_site_index(annotated_site_wlans)

        # 2. SSID Family Analysis — one entry per unique WLAN ID, annotated
        # with all sites it appears at (already resolved by Mist API)
        family_map: dict[str, list[dict]] = defaultdict(list)
        for entry in cross_site.values():
            wlan      = entry["wlan"]
            ssid_name = wlan.get("ssid")
            if not ssid_name:
                continue
            # Create one annotated instance per site for diff analysis
            for sid, site_name in zip(entry["site_ids"], entry["site_names"]):
                instance = dict(wlan)
                instance["_site_name"] = site_name
                instance["_site_id"]   = sid
                family_map[ssid_name].append(instance)
