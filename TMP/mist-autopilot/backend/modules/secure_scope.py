...
class SecureScopeModule(BaseModule):
    ...
    async def analyze(self, org_id: str, sites: list[dict], client: MistClient) -> ModuleOutput:
        ...
        for psk, ssid_set in reused_psks.items():
            ...
            # Suppress if group only differs by band suffix (2.4 / 5 / 6)
            # e.g. "Mist APoS 2.4", "Mist APoS 5", "Mist APoS 6"
            bases = set()
            for s in ssid_list:
                base = s.rstrip(" 0123456789.").rstrip()
                bases.add(base)
            if len(bases) == 1:
                continue  # band variants of the same SSID — expected
        ...
        for site in sites:
            ...
            site_score = self.score_from_findings(site_findings)  # score only site-specific issues
