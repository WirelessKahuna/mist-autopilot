...
**Why we didn't score ourselves at the full 30.** Mist orgs share a uniform API surface, so the module logic behaves identically regardless of customer vertical or size; additional customer orgs would be sources of new insight rather than sources of new defects.
...
- **Security.** Observer token sufficient. Token never written to disk on the server, never logged. Session-only by default; opt-in browser persistence via explicit user checkbox. Per-request throttle, response caching, 8-hour session TTL, no telemetry, no database, no third-party data transmission. Backend endpoints require an active session (no env-var credential fallback); the hosted deployment has no Mist credentials baked in. First-time visitors land on a branded welcome page rather than auto-opening into any org.
