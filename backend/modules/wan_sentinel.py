...
class WANSentinelModule(BaseModule):
    ...
    async def analyze(self, org_id: str, sites: list[dict], client: MistClient) -> ModuleOutput:
        ...
        # ── 1. Fetch per-site gateway stats + org tunnel stats + events + SLEs
        # Gateway stats per site is the primary detection method — finds SSR/SRX
        # regardless of whether hub-spoke tunnels are configured.
        # Org tunnel stats is supplementary for tunnel up/down status detail.
        results = await asyncio.gather(
            client.get_org_wan_tunnels(org_id),
            *[client.get(f"/api/v1/sites/{site['id']}/stats/devices",
                         params={"type": "gateway"},
        ...
        site_gw_results     = results[1      : n+1]
        event_results       = results[n+1    : 2*n+1]
        sle_results         = results[2*n+1  :]

        # ── 2. Build gateway and tunnel maps per site ─────────────────────
        # gateways_by_site: site_id → list of gateway device dicts
        gateways_by_site: dict[str, list[dict]] = defaultdict(list)
        for site, gw_result in zip(sites, site_gw_results):
            if isinstance(gw_result, Exception):
                continue
            gws = gw_result if isinstance(gw_result, list) else []
            for gw in gws:
                if isinstance(gw, dict):
                    gateways_by_site[site["id"]].append(gw)

        # tunnels_by_site from org tunnel stats (supplementary)
        ...
        # ── 3. Check if any WAN devices exist in this org ─────────────────
        any_gateways = any(gateways_by_site.values())
        if not any_gateways and not tunnel_stats:
            return ModuleOutput(
        ...
        for i, site in enumerate(sites):
            ...
            gateways  = gateways_by_site.get(sid, [])
            ...
            if not gateways and not tunnels:
                continue  # no WAN devices at this site — skip silently

            wan_sites_seen.add(sid)

            # ── Check 1: Tunnel down ───────────────────────────────────────
            # Use tunnel stats if available, otherwise check gateway status
            if tunnels:
                ...
                for gw in gateways:
                    gw_name   = gw.get("name") or gw.get("hostname") or gw.get("mac", "?")
                    gw_status = gw.get("status", "")
                    if gw_status in ("disconnected", "inactive") or not gw.get("uptime"):
                        site_findings.append(Finding(
                            severity=Severity.critical,
                            title=f"{site_name} — WAN gateway offline ({gw_name})",
                            detail=(
                                f"Gateway '{gw_name}' at {site_name} is currently "
                                f"{gw_status or 'offline'}. WAN connectivity for this "
                                f"site may be disrupted."
                            ),
                            site_id=sid,
                            site_name=site_name,
                            affected=[gw_name],
                            recommendation=(
                                "Check gateway device power, upstream connectivity, "
