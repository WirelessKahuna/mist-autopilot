...
class SLESentinelModule(BaseModule):
    ...
    async def analyze(self, org_id: str, sites: list[dict], client: MistClient) -> ModuleOutput:
        ...
        async def fetch_metric(
            ...
            nonlocal _debug_logged
            try:
                data = await client.get_site_sle_metric(site_id, scope, metric_key, duration)
                if not _debug_logged and data and isinstance(data, dict):
                    _debug_logged = True
                    logger.info(f"SLE DEBUG top-level keys: {list(data.keys())}")
                    sle_block = data.get("sle", {})
                    logger.info(f"SLE DEBUG sle block keys: {list(sle_block.keys()) if isinstance(sle_block, dict) else type(sle_block)}")
                    samples = sle_block.get("samples", {}) if isinstance(sle_block, dict) else {}
                    logger.info(f"SLE DEBUG samples keys: {list(samples.keys()) if isinstance(samples, dict) else type(samples)}")
                    if isinstance(samples, dict):
                        total = samples.get("total", [])
                        logger.info(f"SLE DEBUG total[:4]: {total[:4] if isinstance(total, list) else total}")
